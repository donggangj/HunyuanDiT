...
class DiffusersPipelineLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "pipeline_folder_name": (os.listdir(HUNYUAN_PATH),),
    ...
    def create_pipeline(self, pipeline_folder_name, model_name, vae_name, backend):
        ...
        if backend == "diffusers":
            ...
            out = load_dit(
                model_path=os.path.join(HUNYUAN_PATH, pipeline_folder_name), MODEL_PATH=MODEL_PATH, VAE_PATH=VAE_PATH
            )
            return (None,) + out[:3]
...
class DiffusersSampler:
    ...
    def sample(self, maked_pipeline, positive, negative, batch_size, width, height, steps, cfg, seed):
        results = maked_pipeline.predict(
            positive,
            height=height,
            width=width,
            seed=int(seed),
