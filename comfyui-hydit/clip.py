    ...
    def encode_from_tokens(self, tokens, return_pooled=False):
        ...
        with torch.no_grad():
            prompt_embeds = self.cond_stage_model(
                tokens["text_input_ids"].to(self.device), attention_mask=attention_mask
            )
            prompt_embeds = prompt_embeds[0]
        t5_attention_mask = tokens["t5_attention_mask"].to(self.device)
        with torch.no_grad():
            t5_prompt_cond = self.embedder_t5(
                tokens["t5_text_input_ids"].to(self.device), attention_mask=t5_attention_mask
            )
...
class HyBertTokenizer:
    ...
    def tokenize(self, text: str):
        ...
        tokens = {"text_input_ids": text_input_ids, "attention_mask": attention_mask}
        return tokens
...
class HyT5Tokenizer:
    ...
    def tokenize(self, text: str):
        ...
        text_input_ids = text_inputs.input_ids
        attention_mask = text_inputs.attention_mask
        tokens = {"t5_text_input_ids": text_input_ids, "t5_attention_mask": attention_mask}
        return tokens
