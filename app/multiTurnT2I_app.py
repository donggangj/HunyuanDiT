...
def pipeline(input_text, state, infer_steps, seed, image_size):
    ...
    if not isinstance(history_messages, list) and len(history_messages.messages) >= 2:
        ...
        response, history_messages = enhancer(
            input_text, return_history=True, history=history_messages, skip_special=False
        )
    ...
    if intention_draw:
        ...
        response = f'<img src="data:image/png;base64,{image_url}" '
        'style="display: inline-block;"><p style="font-size: 14px; color: #555; margin-top: 0;">{prompt}</p>'
...
def upload_image(state, image_input):
    ...
    system_prompt = "请先判断用户的意图，若为画图则在输出前加入<画图>:"
    _ = system_prompt
    history_messages, response = request_mllm(
        question="这张图描述了什么？", history_messages=history_messages, image=image_input.name
    )
...
if __name__ == "__main__":
    ...
    css = """
        ...
        #gallery img:hover {
            transform: scale(2.3); z-index: 999999; position: relative; padding-right: 30%; padding-bottom: 30%;
        }
        #gallery button img:hover {
            transform: none; z-index: 999999; position: relative; padding-right: 0; padding-bottom: 0;
        }
        @media (hover: none) {
    ...
    with gr.Blocks(css=css) as demo:
        DESCRIPTION = """# <a style="color: black; text-decoration: none;">多轮对话绘图 Multi-turn Text2Image Generation</a>
            ...
            (You can modify historical images through simple interactive statements
            referred to [DialogGen](https://arxiv.org/abs/2403.08857), such as:
            enity edit, add object, remove object, change background, change style, change lens, and combine images.)
        ...
        with gr.Row(), gr.Column(scale=1, min_width=1000):
            with gr.Row():
            ...
            with gr.Row():
                ...
                size_dropdown = gr.Dropdown(
                    choices=["正方形(square, 1024x1024)", "风景(landscape, 1280x768)", "人像(portrait, 768x1280)"],
            ...
            with gr.Row():
                ...
                with gr.Column():
                    submit_btn = gr.Button("提交(Submit)", interactive=True, variant="primary")
            ...
            gr.Markdown(
                """<p style="font-size: 20px; color: #888;">powered by
                <a href="https://github.com/Centaurusalpha/DialogGen" target="_blank">DialogGen</a>
                and <a href="https://github.com/Tencent/HunyuanDiT" target="_blank">HunyuanDiT</a></p>"""
            )
    ...
    interface = demo
    interface.launch(server_name="0.0.0.0", server_port=6666, share=False)
