...
def get_start_epoch(resume_path, ckpt, steps_per_epoch):
    ...
    if "steps" in ckpt:
        ...
        try:
            ...
            print(f"Cannot parse train steps from resume path: {err}")
...
def model_resume(args, model, ema, logger, len_loader):
    ...
    if args.resume_deepspeed:
        ...
        resume_ckpt = torch.load(
            os.path.join(resume_path, "mp_rank_00_model_states.pt"), map_location=lambda storage, loc: storage
        )
        # Resume main model
    ...
    # Resume model and ema states (not include optimizer states) from two checkpoints separated from DeepSpeed ckpt.
        ...
        if args.ema_to_module:
            ...
            logger.info("    Resume main model from ema states.")
            resume_ckpt_ema = torch.load(
                os.path.join(resume_path, "pytorch_model_ema.pt"), map_location=lambda storage, loc: storage
            )
            ...
            logger.info("    Resume main model from main states.")
            resume_ckpt_module = torch.load(
                os.path.join(resume_path, "pytorch_model_module.pt"), map_location=lambda storage, loc: storage
            )
        ...
        if args.use_ema:
            if args.module_to_ema:
                if "resume_ckpt_module" in locals():
                    logger.info("    Resume ema model from main states.")
                    ema.load_state_dict(resume_ckpt_module, strict=args.strict)
                else:
                    assert "pytorch_model_module.pt" in os.listdir(
                        resume_path
                    ), f"    Cannot find pytorch_model_module.pt from {resume_path}"
                    logger.info("    Resume ema model from module states.")
                    resume_ckpt_module = torch.load(
                        os.path.join(resume_path, "pytorch_model_module.pt"), map_location=lambda storage, loc: storage
                    )
                    ema.load_state_dict(resume_ckpt_module, strict=args.strict)
                ...
                if "resume_ckpt_ema" in locals():
                    logger.info("    Resume ema model from EMA states.")
