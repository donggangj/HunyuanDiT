...
def deepspeed_initialize(args, logger, model, opt, deepspeed_config):
    ...
    model, opt, _, scheduler = deepspeed.initialize(
        ...
        args=args,
        lr_scheduler=(
            partial(get_learning_rate_scheduler, args.warmup_min_lr, args.lr, args.warmup_num_steps)
...
def save_checkpoint(args, rank, logger, model, ema, epoch, train_steps, checkpoint_dir):
    ...
    if train_steps % args.ckpt_every == 0:
        ...
        try:
            ...
            logger.error(f"Saved failed to {checkpoint_path}: {err}")
...
def main(args):
    ...
    assert args.deepspeed, "Must enable deepspeed in this script: train_deepspeed.py"
    ...
    logger.info(
        f"      Batch size all device:     {batch_size * world_size * grad_accu_steps:,} "
        f"(world_size * batch_size * grad_accu_steps)"
    )
    ...
    logger.info(
        ...
        f"{train_steps:,}/{min(args.max_training_steps, args.epochs * iters_per_epoch):,}"
    )
    logger.info("    ------------------------------------------------------------------------------")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, batch in enumerate(loader):
            ...
            if args.use_fp16 and args.use_ema and args.async_ema:
            ...
            if (
                ...
                or train_steps % args.ckpt_latest_every == 0  # or train_steps == args.max_training_steps
            ) and train_steps > 0:
