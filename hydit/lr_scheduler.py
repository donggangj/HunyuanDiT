...
def add_tuning_arguments(parser):
    ...
    group.add_argument(
        "--lr_range_test_staircase", type=bool, default=False, help="use staircase scaling for LR range test."
    )
    ...
    group.add_argument("--cycle_first_stair_count", type=int, default=-1, help="first stair count for 1Cycle schedule.")
    group.add_argument(
    ...
    group.add_argument(
        "--cycle_second_stair_count", type=int, default=-1, help="second stair count for 1Cycle schedule."
    )
    group.add_argument(
        "--decay_step_size",
        type=int,
    ...
    group.add_argument(
        "--warmup_type", type=str, default=WARMUP_LOG_RATE, help="WarmupLR increasing function during warmup"
    )
    return parser
...
class LRRangeTest:
    ...
    Example:
        ...
        _A disciplined approach to neural network hyper-parameters: Part 1 -- learning rate, batch size, momentum,
            and weight decay:
...
class OneCycle:
    ...
    .. _A disciplined approach to neural network hyper-parameters: Part 1 -- learning rate, batch size, momentum,
            and weight decay: https://arxiv.org/abs/1803.09820
    """
    ...
    def __init__(
        ...
        if cycle_momentum:
            self._initialize_momentum(
                self.optimizer, cycle_min_mom, cycle_max_mom, decay_mom_rate, last_batch_iteration
            )
    ...
    def _initialize_cycle(
        ...
        cycle_second_step_size = (
            float(cycle_second_step_size) if cycle_second_step_size is not None else cycle_first_step_size
        )
    ...
    def _initialize_momentum(self, optimizer, cycle_min_mom, cycle_max_mom, decay_mom_rate, last_batch_iteration):
        if "betas" not in optimizer.defaults:
            ...
            print(
                f"cycle_momentum is disabled because optimizer {optimizer_name} "
                "does not support momentum, no betas attribute in defaults"
            )
...
class WarmupLR:
    ...
    def __init__(
        ...
        if warmup_type not in {WARMUP_LOG_RATE, WARMUP_LINEAR_RATE}:
            print(f"Using unknown warmup_type: {warmup_type}. The increasing function " f"is set to default (log)")
            ...
            if len(param_value) != len(optimizer.param_groups):
                raise ValueError(
                    "expected {} value for {}, got {}".format(
                        len(optimizer.param_groups), param_name, FileNotFoundError(param_value)
                    )
                )
...
class WarmupDecayLR(WarmupLR):
    ...
    def __init__(
        ...
        if self.total_num_steps < self.warmup_num_steps:
            print("total_num_steps {} is less than warmup_num_steps {}".format(total_num_steps, warmup_num_steps))
