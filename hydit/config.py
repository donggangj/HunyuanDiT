...
from .constants import NOISE_SCHEDULES, PREDICT_TYPE, SAMPLER_FACTORY
...
def get_args(default_args=None):
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[1024, 1024],
        help="Image size (h, w). If a single value is provided, the image will be treated to " "(value, value).",
    )
    parser.add_argument(
        "--qk-norm",
        action="store_true",
    ...
    parser.add_argument(
        "--predict-type", type=str, choices=list(PREDICT_TYPE), default="v_prediction", help="Diffusion predict type"
    )
    parser.add_argument(
        "--noise-schedule", type=str, choices=list(NOISE_SCHEDULES), default="scaled_linear", help="Noise schedule"
    )
    ...
    parser.add_argument(
        "--target-ratios", type=str, nargs="+", default=None, help="Target ratios for multi-resolution training."
    )
    ...
    # Acceleration
    parser.add_argument(
        "--infer-mode", type=str, choices=["fa", "torch", "trt"], default="torch", help="Inference mode"
    )
    ...
    parser.add_argument(
        "--ckpt-latest-every", type=int, default=10_000, help="Create a ckpt named `latest.pt` every a few steps."
    )
    ...
    parser.add_argument(
        "--rope-real", action="store_true", help="Use real part and imaginary part separately for RoPE."
    )
    ...
    parser.add_argument("--ema-to-module", action="store_true", help="If true, initialize the module with EMA weights.")
    parser.add_argument("--module-to-ema", action="store_true", help="if true, initialize the ema with Module weights.")

    # Dataset
    parser.add_argument(
        "--index-file", type=str, nargs="+", help="During training, provide a JSON file with data indices."
    )
    ...
    parser.add_argument(
        "--random-shrink-size-cond", action="store_true", help="Randomly shrink the original size condition."
    )
    ...
    parser.add_argument(
        "--ema-decay", type=float, default=None, help="EMA decay rate. If None, use the default value of the model."
    )
    parser.add_argument(
        "--ema-warmup", action="store_true", help="EMA warmup. If True, perform ema_decay warmup from 0 to ema_decay."
    )
    parser.add_argument(
        "--ema-warmup-power", type=float, default=None, help="EMA power. If None, use the default value of the model."
    )
    parser.add_argument(
        "--ema-reset-decay",
        action="store_true",
        help="Reset EMA decay to 0 and restart increasing the EMA decay." "Only works when --ema-warmup is enabled.",
    )
    # Acceleration
    parser.add_argument(
        "--use-flash-attn",
