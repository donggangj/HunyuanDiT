    ...
    def forward(
        ...
        if batch_size <= self.max_batch_size:
            ...
            embeddings_inp = cuda.DeviceView(
                ptr=context_c.reshape(-1).data_ptr(), shape=context_c.shape, dtype=np.float16
            )
            image_meta_size_inp = cuda.DeviceView(
                ptr=image_meta_size_c.reshape(-1).data_ptr(), shape=image_meta_size_c.shape, dtype=np.float16
            )
            style_inp = cuda.DeviceView(ptr=style_c.reshape(-1).data_ptr(), shape=style_c.shape, dtype=np.int64)
            freqs_cis_img0_inp = cuda.DeviceView(
                ptr=freqs_cis_img0_c.reshape(-1).data_ptr(), shape=freqs_cis_img0_c.shape, dtype=np.float32
            )
            freqs_cis_img1_inp = cuda.DeviceView(
                ptr=freqs_cis_img1_c.reshape(-1).data_ptr(), shape=freqs_cis_img1_c.shape, dtype=np.float32
            )
            text_embedding_mask_inp = cuda.DeviceView(
                ptr=text_embedding_mask_c.reshape(-1).data_ptr(), shape=text_embedding_mask_c.shape, dtype=np.int64
            )
            encoder_hidden_states_t5_inp = cuda.DeviceView(
                ptr=encoder_hidden_states_t5_c.reshape(-1).data_ptr(),
                shape=encoder_hidden_states_t5_c.shape,
                dtype=np.float16,
            )
            text_embedding_mask_t5_inp = cuda.DeviceView(
                ptr=text_embedding_mask_t5_c.reshape(-1).data_ptr(),
                shape=text_embedding_mask_t5_c.shape,
                dtype=np.int64,
            )
            feed_dict = {
                "x": sample_inp,
                ...
                "style": style_inp,
                "cos_cis_img": freqs_cis_img0_inp,
