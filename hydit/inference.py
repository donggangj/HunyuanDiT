...
class ResolutionGroup:
    def __init__(self):
        ...
        self.supported_sizes = {(r.width, r.height) for r in self.data}
...
def get_pipeline(args, vae, text_encoder, tokenizer, model, device, rank, embedder_t5, infer_mode, sampler=None):
    """
...
class End2End:
    def __init__(self, args, models_root_path):
        ...
        if self.infer_mode in ["fa", "torch"]:
            ...
            logger.info("Loading TensorRT model finished")
        else:
    ...
    def predict(
        ...
        freqs_cis_img = self.freqs_cis_img.get(reso, self.calc_rope(target_height, target_width))
