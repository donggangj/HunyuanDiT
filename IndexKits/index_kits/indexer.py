...
class ArrowIndexV2:
    ...
    def __init__(self, index_file=None, res_dict=None, align=1, shadow_file_fn=None, **kwargs):
        if index_file is not None:
            ...
            raise ValueError("Either index_file or res_dict should be provided.")
        ...
        if len(self.arrow_files) != len(self.cum_length):
            raise ValueError(
                f"Length of arrow_files and cum_length does not match. "
                f"{len(self.arrow_files)} != {len(self.cum_length)}"
            )
        if len(self.arrow_files) != len(self.group_length):
            raise ValueError(
                f"Length of arrow_files and group_length does not match. "
                f"{len(self.arrow_files)} != {len(self.group_length)}"
            )
        if len(self.indices) == 0:
            raise ValueError("No indices found in index_dict.")
        if isinstance(self.indices, list) and self.indices[-1] > self.cum_length[-1] - 1:
            raise ValueError("Indices exceed cum_length.")
    ...
    def align(self, align):
        ...
        if repeat_num >= len(self):
            ...
            group_length_new = list(self.group_length)
    ...
    def load_table_by_index(self, index, shadow=None):
        if shadow is None:
            if index == self.last_index:
                return self._cur_table
            arrow_file, self._index_bias = self.get_arrow_file_by_index(index, return_index_bias=True)
    ...
    def get_data(self, ind, columns=None, allow_missing=False, return_meta=True, shadow=None):
        ...
        return self.get_data_by_index(
            index, columns, allow_missing=allow_missing, return_meta=return_meta, shadow=shadow
        )
    ...
    def __init__(
        ...
        if len(self.group_length) > 0:
            ...
            if sum(self.group_length) != len(self.indices):
                raise ValueError(
                    f"Sum of group_length does not match length of indices. "
                    f"{sum(self.group_length)} != {len(self.indices)}"
                )
