...
def show_index_info(src, only_arrow_files=False, depth=1):
    ...
    if len(arrow_files) <= 4:
        arrow_files_repr = return_space.join(list(arrow_files))
    else:
        arrow_files_repr = return_space.join(list(arrow_files[:2]) + ["..."] + list(arrow_files[-2:]))
