...
class MultiIndexV2:
    ...
    def __init__(
        ...
        self.buckets = self.load_buckets(
            index_files, batch_size=batch_size, world_size=world_size, shadow_file_fn=shadow_file_fn
        )
    ...
    def check_sample_strategy(self, sample_strategy, probability):
        if sample_strategy == "uniform":
            ...
            if probability is None:
                raise ValueError("probability must be provided when sample_strategy is 'probability'.")
            assert isinstance(probability, (list, tuple)), f"probability must be a list, but got {type(probability)}"
    ...
    def get_data(self, ind, columns=None, allow_missing=False, return_meta=True, **kwargs):
        ...
        return self.buckets[i].get_data(
            ind - bias, columns=columns, allow_missing=allow_missing, return_meta=return_meta, **kwargs
        )
...
class MultiMultiResolutionBucketIndexV2(MultiIndexV2):
    ...
    def load_buckets(self, index_files, **kwargs):
        ...
        if self.batch_size is None or self.world_size is None:
            raise ValueError(
                "`batch_size` and `world_size` must be provided when using " "`MultiMultiResolutionBucketIndexV2`."
            )
        buckets = [
            MultiResolutionBucketIndexV2(
                ...
                self.batch_size,
                self.world_size,
                shadow_file_fn=kwargs.get("shadow_file_fn"),
            )
    ...
    def sample_indices_with_probability(self, return_batch_indices=False):
        ...
        for bucket, p in zip(self.buckets, self.probability):
            if p == 1:
                ...
                indices_part2 = (
                    self.sampler.choice(len(bucket) // bs, int(len(bucket) * (p / bs - repeat_times)), replace=False)
                    * bs
                )
